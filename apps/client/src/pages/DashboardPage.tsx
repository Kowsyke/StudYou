...
import { daysLeftLabel, formatDate, formatGbp, formatHome, safeExternalUrl } from '../lib/format'
...
export function DashboardPage() {
  ...
  useGSAP(() => {
    ...
    // greeting immediately, with no scramble. Entrance for the content blocks
    // is handled by the shared Reveal primitives, which read the same
    // preference through MotionConfig.
  ...
  return (
    <div>
      <header className="mb-6">
        <h1 ref={greetingRef} className="text-title3 font-bold text-ink scramble-active">
          Hello, &nbsp;
        </h1>
        <p className="text-xs text-ink-secondary mt-1.5 max-w-prose">
          ...
          <Card className="card-lift h-full">
            ...
            <CardContent className="flex flex-col gap-4">
              {percentComplete > 0 ? (
                ...
                /* At zero there is no arc to draw, so the plain ring reads as
                   an unstyled grey disc. This state says so on purpose: a
                   visible track, a start marker, and a not started label. */
                <div className="flex items-center gap-6">
                  <div className="relative h-[90px] w-[90px] shrink-0">
                    <div
                      className="absolute inset-0 rounded-full border-[8px] border-hairline-strong bg-canvas"
                      aria-hidden="true"
                    />
                    {/* Start marker at twelve o'clock: the point the arc grows from. */}
                    <span
                      className="absolute left-1/2 top-[-1px] h-2.5 w-2.5 -translate-x-1/2 rounded-full bg-accent"
                      aria-hidden="true"
                    />
                    {/* Carries the same test id as ProgressRing so the completion
                        figure stays readable in both states, at zero and above. */}
                    <span
                      data-testid="percent-complete"
                      className="absolute inset-0 flex items-center justify-center text-title3 font-bold text-ink-tertiary tabular-nums"
                    >
                      0%
                    </span>
                  </div>
                  <div className="flex flex-col gap-1 min-w-0">
                    <span className="text-body-lg font-bold text-ink">Not started yet</span>
                    <span className="text-caption text-ink-secondary">
                      {taskTotal === 0
                        ? 'Your roadmap has no tasks yet.'
                        : `None of your ${taskTotal} tasks are ticked off. The ring fills in as you complete them.`}
                    </span>
                  </div>
                </div>
              )}
              <Link
                to="/journey"
                className="text-caption font-semibold text-accent hover:underline self-start"
              >
                {percentComplete > 0 ? 'Open your roadmap' : 'Start your roadmap'} &rarr;
              </Link>
            </CardContent>
          </Card>
        </RevealItem>
        ...
        <RevealItem>
          <Card className="card-lift h-full">
            ...
            <CardContent className="flex flex-col gap-3">
              ...
              <div className="h-2 bg-surface-secondary border border-hairline rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-[width] duration-300 ease-[var(--ease-swift)] ${
                    budget.overBudget ? 'bg-danger' : 'bg-accent'
                  }`}
                        ...
                        {(() => {
                          ...
                          return (
                            <Link
                              ...
                              className="inline-flex items-center gap-1 text-caption font-medium text-accent hover:underline mt-1"
                            >
                      ...
                      <div className="flex items-baseline justify-between gap-3 text-xs">
                        ...
                        <span
                          className={`shrink-0 tabular-nums font-semibold ${
                            complete ? 'text-positive' : 'text-ink-secondary'
                          }`}
                        >
                          {s.done} of {s.total}, {percent}%
                        </span>
                      </div>
                      {/* A taller track with a hairline edge, so a stage sitting
                        at zero still reads as an empty bar rather than nothing. */}
                      <ProgressBar
                        value={percent}
                        className="h-2 bg-surface-secondary border border-hairline"
                      />
                    </div>
                  )
                })}
                      ...
                      >
                        <div className="flex gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-caption text-ink-secondary line-clamp-2 mt-0.5 leading-relaxed">
                              {res.summary}
                            </p>
                            <div className="flex items-center gap-2 mt-2 text-micro text-ink-tertiary">
                        ...
                        <div className="flex flex-col gap-1 items-end shrink-0">
                          ...
                          <a
                            href={safeExternalUrl(res.sourceUrl)}
                            target="_blank"
                            rel="noreferrer"
...
function StatTile({
  ...
  return (
    <Card className="dashboard-stat-tile card-lift h-full p-4 flex items-center gap-3 cursor-default">
